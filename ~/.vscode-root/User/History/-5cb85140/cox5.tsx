...
import Image from 'next/image'; // 💡 Image ကို ပြန်ခေါ်ထားပါတယ်
    ...
    <nav className={`fixed top-0 left-0 w-full z-50 transition-all duration-500 ${isScrolled ? 'bg-white/80 dark:bg-[#091830]/80 backdrop-blur-xl border-b border-gray-200 dark:border-white/10 shadow-sm py-4' : 'bg-transparent py-6 md:py-8'}`}>
      <div className="max-w-7xl mx-auto px-6 md:px-12 lg:px-20 w-full">
        <div className="flex items-center justify-between">
          ...
          {/* 💡 Logo ပုံအစစ် ပြန်ခေါ်ထားတဲ့အပိုင်း */}
          {/* Logo ပြမည့် နေရာ */}
          <Link href="/" className="flex items-center">
            
            {/* ☀️ Light Mode အတွက် Logo (Dark Mode ရောက်ရင် ပျောက်သွားမည်) */}
            <Image 
              src="/logo-light.png" 
              alt="Nyanetic Logo Light" 
              width={150} 
              height={40} 
              className="w-auto h-8 md:h-10 object-contain block dark:hidden"
              priority
            />

            {/* 🌙 Dark Mode အတွက် Logo (Light Mode ရောက်ရင် ပျောက်သွားမည်) */}
            <Image 
              src="/logo-dark.png" 
              alt="Nyanetic Logo Dark" 
              width={150} 
              height={40} 
              className="w-auto h-8 md:h-10 object-contain hidden dark:block"
              priority
            />
